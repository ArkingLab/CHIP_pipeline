...
fin_df <- read.delim("Input.vcf", skip = 1)#skip until the X.CHROM line
...
fin_df <- fin_df[!(fin_df$AltF1R2 < 2),]
fin_df <- fin_df[!(fin_df$AltF2R1 < 2),]
...
fin_df$VAF_adj <- ifelse(fin_df$X.CHROM == "chrX" & fin_df$sex == "men", fin_df$newVAF/2, fin_df$newVAF)


fin_df <- fin_df[!(fin_df$VAF_adj > 20),]

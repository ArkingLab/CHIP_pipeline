...
whitelist.mis<-read.csv("LCHIPPATH_CHIP_missense_vars_Bick_practical.csv")

whitelist.splice<-read.csv("LCHIPPATH_CHIP_splice_vars_Bick_practical.csv")

whitelist.LoF<-read.csv("LCHIPPATH_CHIP_nonsense_FS_vars_Bick_practical.csv")



vars1<-read.csv("PathogenicOut.hg38_multianno.csv")
...
fin_df$SampID <- str_extract(fin_df$INFO, "SM=[0-9]*")%>%
  str_remove(., "SM=")%>%
  as.character()
...
fin_df$Func.refGene[fin_df$transcriptOI %in% ManCheck$transcriptOI]
fin_df$ExonicFunc.refGene[fin_df$transcriptOI %in% ManCheck$transcriptOI]

fin_df$transcriptOI[fin_df$transcriptOI %in% ManCheck$transcriptOI & fin_df$Func.refGene == "exonic"]


NDMC <- read.csv("NODEL_ManualCheck.csv")

ManCheck <- ManCheck[!ManCheck$transcriptOI %in% NDMC$transcriptOI,]
write.csv(ManCheck, file = "ManCheckOut.csv", row.names = FALSE)
...
fin_df$DiscordantLof <- ((fin_df$Gene.refGene %in% c(whitelist.LoF$Gene)) & fin_df$DiscordantLof)
...
DiscordantLofCheck <- DiscordantLofCheck[!DiscordantLofCheck$transcriptOI %in% NDMC$transcriptOI,]

write.csv(DiscordantLofCheck, file = "DiscordantLofCheck.csv", row.names = FALSE)

write.table(fin_df, file = "Pathogenic_SemiFilteredDF.tsv", quote = FALSE, row.names = FALSE, sep = "\t")
